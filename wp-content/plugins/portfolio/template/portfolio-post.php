					...
					<div class="portfolio_content entry-content">
						<div class="entry">
							...
							<div class="col-sm-4 ">
									...
									<img src="<?php echo esc_url($image_large[0]); ?>"  alt="<?php echo esc_attr($image_alt); ?>" />

							</div><!-- .portfolio_thumb -->
							<div class="portfolio_content col-sm-8">
								<?php if ( 1 == $portfolio_options['prtfl_date_additional_field'] ) { ?>
										<p>
											<span class="lable"><?php echo esc_html($portfolio_options['prtfl_date_text_field']); ?></span> <?php echo $date_compl; ?>
										</p>
								<?php }
								...
								if ( 1 == $portfolio_options['prtfl_link_additional_field'] ) {
									if ( false !== parse_url( $link ) ) { ?>
										<?php if ( ( 0 == $user_id && 0 == $portfolio_options['prtfl_link_additional_field_for_non_registered'] ) || 0 != $user_id ) { ?>
											<p><span class="lable"><?php echo $portfolio_options['prtfl_link_text_field']; ?></span> <a href="<?php echo esc_url($link); ?>"><?php echo esc_html($link); ?></a></p>
										<?php } else { ?>
							...
							if ( is_array( $terms ) && 0 < count( $terms ) ) { ?>
								<div class="portfolio_terms"><?php echo $portfolio_options['prtfl_technologies_text_field']; ?>
									...
									foreach ( $terms as $term ) {
										if ( $count > 0 )
											echo ', ';
										echo '<a href="' . esc_url(get_term_link( $term->slug, 'portfolio_technologies')) . '" title="' . sprintf( __( "View all posts in %s","portfolio"), $term->name ) . '" ' . '>' . esc_html($term->name) . '</a>';
										$count++;
									} ?>
								</div><!-- .portfolio_terms -->
							<?php } ?>
									...
									<?php } ?>
										<div class="portfolio_images_gallery">
											<a class="lightbox" rel="portfolio_fancybox" href="<?php echo esc_url($image_large[0]); ?>" title="<?php echo esc_attr($image_desc); ?>">
												<img src="<?php echo esc_url($image[0]); ?>" width="<?php echo esc_attr($portfolio_options['prtfl_custom_size_px'][1][0]); ?>" height="<?php echo esc_attr($portfolio_options['prtfl_custom_size_px'][1][1]); ?>" alt="<?php echo esc_attr($image_alt); ?>" />
											</a>
