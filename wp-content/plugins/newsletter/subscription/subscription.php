...
class NewsletterSubscription extends NewsletterModule {
    ...
    function subscribe($status = null, $emails = true) {
		...
		if(isset($this->options['noconfirmation'])){
		...
		$_REQUEST['ns']=(isset($_REQUEST['ns']))?$_REQUEST['ns']:"";
		...
		$_REQUEST['nl']=(isset($_REQUEST['nl']))?$_REQUEST['nl']:"";
        ...
        if (empty($options[$prefix . 'disabled'])) {
            ...
            $message_text = isset($options[$prefix . 'message_text'])?$options[$prefix . 'message_text']:"";
    ...
    function mail($to, $subject, $message) {
		...
		$this->options['template_enabled']=(isset($this->options['template_enabled']))?$this->options['template_enabled']:0;
...
function newsletter_register_form() {
    $module = NewsletterSubscription::instance();
	if(isset($module->options['subscribe_wp_users'])){
