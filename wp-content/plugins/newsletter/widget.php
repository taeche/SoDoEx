...
class NewsletterWidget extends WP_Widget {
    ...
    static function get_widget_form() {
        ...
        // Referrer
        $form .= '<input type="hidden" name="nr" value="widget"/><div class="rowform"><i class="olv olv-mail"></i>'; // Fix Olivia

        if ($options_profile['name_status'] == 2)
            $form .= '<input class="newsletter-firstname" type="text" name="nn" value="' . esc_attr($options_profile['name']) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/>';

        if ($options_profile['surname_status'] == 2)
            $form .= '<input class="newsletter-lastname" type="text" name="ns" value="' . esc_attr($options_profile['surname']) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/>';

        $form .= '<input class="newsletter-email" type="email" required name="ne" value="' . esc_attr($options_profile['email']) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/>';
        ...
        for ($i = 1; $i <= NEWSLETTER_PROFILE_MAX; $i++) {
            ...
            if ($options_profile['profile_' . $i . '_type'] == 'text') {
                $form .= '<p><input class="newsletter-profile newsletter-profile-' . esc_attr($i) . '" type="text" name="np' . esc_attr($i) . '" value="' . esc_attr($options_profile['profile_' . $i]) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/></p>';
            }
            if ($options_profile['profile_' . $i . '_type'] == 'select') {
                $form .= '<p>' . esc_attr($options_profile['profile_' . $i]) . '<br /><select class="newsletter-profile newsletter-profile-' . esc_attr($i) . '" name="np' . esc_attr($i) . '">';
                $opts = explode(',', $options_profile['profile_' . $i . '_options']);
                for ($t = 0; $t < count($opts); $t++) {
                    $form .= '<option>' . esc_html(trim($opts[$t])) . '</option>';
                }
                $form .= '</select></p>';
            }
        }
        ...
        if (strpos($options_profile['subscribe'], 'http://') !== false) {
            $form .= '<input class="newsletter-submit" type="image" src="' . esc_url($options_profile['subscribe']) . '"/>';
        } else {
            $form .= '<input class="btn newsletter-submit btn-primary" type="submit" value="' . esc_attr($options_profile['subscribe']) . '"/></div>';
        }
