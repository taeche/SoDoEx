...
<div <?php echo esc_attr( ! empty( $args['cssID'] ) ? 'id="' . sanitize_html_class( $args['cssID'] ) . '"' : '' )?> class="rpwe-block <?php echo esc_attr( ! empty( $args['css_class'] ) ? '' . sanitize_html_class( $args['css_class'] ) . '' : '' )?>">
    ...
    <?php endif; ?> </h2>
 <?php endif; ?>
    <div class="rpwe-div focus-top slider blog-grid space20 cursor-move">
		<?php  $row_count=3;
			$count=0;
			?>
		<?php while ( $posts->have_posts() ) : $posts->the_post();
			...
			echo (($count-1)%$row_count==0)?"<div class='rpwe-li item'>":"";?>

				<div class="innerItem  <?php  echo ($count%$row_count == 1)? 'top-item' : 'more-item'; ?>">
					<?php if ( $args['thumb'] ) : ?>
						...
						<div class="item-desc-header">
							<div class="readmore-comment pull-right">
									<?php if( intval( $args[ 'dcomment' ] ) ) : 
										...
										<?php echo sprintf('<a href="%1$s" class="moduleItemComments"><i class="fa fa-comments"></i> %2$s</a>', $clink, $comments);
									endif;
						  ...
						  <?php if ( $args['date'] ) : ?>
							...
							<?php endif; //endif ($count-1)%$row_count == 0 ?>
                            
						</div>  <!-- end item-desc-header  -->

						<?php if (  isset( $args['dtitle'] ) && intval( $args['dtitle'] ) ) : ?>
                        <h3 class="rpwe-title"><a href="<?php echo esc_url( get_permalink() )?>" title="<?php echo sprintf( esc_attr__( 'Permalink to %s', 'jv_allinone' ), the_title_attribute( 'echo=0' ) )?>" rel="bookmark"><?php echo esc_attr( get_the_title() ) ?></a></h3>
						<?php endif; ?>
			...
			<?php
				echo ($count%$row_count==0||$count==$posts->found_posts)?"</div>":"";
			 ?>
