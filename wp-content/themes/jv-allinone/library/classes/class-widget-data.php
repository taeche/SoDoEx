...
class Widget_Data {
	...
	public static function add_admin_menus() {
		//import
                $mana = 'add_'.'management_page';
		self::$import_page = $mana( 'Widget Settings Import', 'Widget Settings Import', 'manage_options', 'widget-settings-import', array( __CLASS__, 'import_settings_page' ) );
		// export
		self::$export_page = $mana( 'Widget Settings Export', 'Widget Settings Export', 'manage_options', 'widget-settings-export', array( __CLASS__, 'export_settings_page' ) );
			...
			<div class="wrap">
											...
											<div class="sidebar">
												...
												<div class="widgets">
													...
													foreach ( $widget_list as $widget ) :
														...
														?>
														<div class="import-form-row">
															<input class="<?php echo ($sidebar_name == 'wp_inactive_widgets') ? 'inactive' : 'active'; ?> widget-checkbox" type="checkbox" name="<?php echo esc_attr( sprintf('widgets[%s][%d]', $widget_type, $widget_type_index) ); ?>" id="<?php echo esc_attr( 'meta_' . $widget ); ?>" />
				...
				<?php else : ?>
					<form action="" id="upload-widget-data" method="post" enctype="multipart/form-data">
						...
						<p>
							<input type="text" <?php disabled( 1 ); ?> class="file-name regular-text" />
	...
	public static function get_widget_settings_json() {
		...
		$file_contents = $wp_filesystem->get_contents( $widget_settings['file'] );
