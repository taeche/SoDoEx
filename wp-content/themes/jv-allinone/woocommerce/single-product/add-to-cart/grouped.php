...
<form class="cart cart-grouped borderBottom" method="post" enctype='multipart/form-data'>
					...
					<tr>
						...
						<td class="label">
							<label for="product-<?php echo esc_attr($product_id); ?>">
								<?php echo apply_filters( 'esc_html', $product->is_visible() ? '<a href="' . get_permalink() . '">' . get_the_title() . '</a>' : get_the_title() ); ?>
							</label>
						</td>
						...
						<td class="price">
							<?php
								echo apply_filters( 'esc_html', $product->get_price_html() );
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button btn btn-primary alt"><i class="icon-cart42"></i> <?php echo apply_filters( 'esc_html', $product->single_add_to_cart_text()); ?></button>
