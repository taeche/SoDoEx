		...
		if( is_array($array) ){
			...
			foreach( $array as $slide ){
				...
				if( $the_bg || $extra || $video_embed ){
					...
					$buttons = ($link || $link_2) ? sprintf( '<div class="slider-buttons" style="margin-bottom:1em">%s %s</div>', $link, $link_2 ) : '';

				
					if( ! $extra ){
						
						$caption = sprintf(
								'<div class="caption slider-content %s" data-x="%s" data-y="bottom" data-speed="300" data-start="500" data-easing="easeOutExpo">%s %s</div>',
								$caption_class,
								$the_x,
								$content,
								$buttons
